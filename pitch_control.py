'''
This module contains all the pitch control calculations and the paramaters for animations


Credit: This module is slightly modified version from Metrica_Viz.py made available by Friends Of Tracking community
...
def initialise_players(team,teamname,params,GK_jersey):
    """
    ...
    """    
    # get player  ids
    player_jerseys = get_player_jersey_from_tracking(team)
    #player_ids = np.unique( [ c.split('_')[1] for c in team.keys() if c[:4] == teamname ] )
    # create list
    team_players = []
    for p in player_jerseys:
        # create a player object for player_id 'p'
        team_player = player(p,team,teamname,params,GK_jersey)
...
class player(object):
    ...
    def __init__(self,jersey_no,team,teamname,params,GKid):
        self.id = jersey_no
        self.is_gk = self.id == GKid
        self.teamname = teamname
        self.playername = '{}_'.format(jersey_no)
    ...
    def get_position(self,team):
        self.position = np.array( [ float(team[self.playername+'x']), float(team[self.playername+'y'])] )
        self.inframe = not np.any( np.isnan(self.position) )
        
    def get_velocity(self,team):
        self.velocity = np.array( [float(team[self.playername+'vx']), float(team[self.playername+'vy'])])
...
def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params, GK_numbers, field_dimen = (105.,68.,), n_grid_cells_x = 50, offsides=True):
    ...
    ball_start_pos = np.array([(float(events[event_id]['Start X'])-0.5) *field_dimen[0],-(float(events[event_id]['Start Y'])-0.5)*field_dimen[1]])
    ...
    if pass_team=='Home':
        attacking_players = initialise_players(tracking_home[pass_frame],'Home',params,GK_numbers[0])
        defending_players = initialise_players(tracking_away[pass_frame],'Away',params,GK_numbers[1])
    elif pass_team=='Away':
        defending_players = initialise_players(tracking_home[pass_frame],'Home',params,GK_numbers[0])
        attacking_players = initialise_players(tracking_away[pass_frame],'Away',params,GK_numbers[1])
    else:
...
def generate_pitch_control_for_frame(events, tracking_home, tracking_away, params, GK_numbers, field_dimen = (105.,68.,), n_grid_cells_x = 25, offsides=True):
    '''
    Evaluates pitch control surface over the entire field at the moment of the given frame 
          
    '''
    #Get the pass frame number from the given frame in dataFrame format
    pass_frame = int(tracking_home['Frame'])
    
    #Get the pass team with ball possesion for the frame     
    pass_team = events[events['Start Frame'] <= pass_frame][len(events['Start Frame'] <= pass_frame)-1]['Team']
    
    ball_start_pos = np.array([(float(tracking_home['Ball_x'])),(float(tracking_home['Ball_y']))])
